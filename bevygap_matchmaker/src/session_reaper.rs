...
pub(crate) async fn session_cleanup_supervisor(
    ...
    queue_session_deletes: bool,
) -> Result<(), async_nats::Error> {
    let state = orig_state.clone();
    let handle1 = tokio::spawn(async move {
        loop {
            let _ = session_cleanup_watcher(&state, queue_session_deletes).await;
    ...
    let handle2 = tokio::spawn(async move {
        loop {
            let _ = unclaimed_session_reaper(&state, queue_session_deletes).await;
...
async fn unclaimed_session_reaper(
    ...
    loop {
        ...
        while let Some(key) = keys.try_next().await? {
            ...
            let session_id = match String::from_utf8(entry.value.to_vec()) {
                Ok(value) => value,
                Err(error) => {
                    warn!("Ignoring unclaimed session entry {key}: value is not utf8: {error}");
                    continue;
                }
            };
            ...
            if age > Duration::from_secs(crate::MAX_SESSION_CREATION_SECONDS + 2) {
                ...
                if queue_session_deletes {
                    ...
                    info!("Mock mode: not queueing delete for unclaimed session {session_id}");
                }
                kv.delete(&key).await?;
            }
        }
        ...
        match event {
            Ok(event) => {
                ...
                if event.operation == Operation::Delete {
                    info!("Active connection delete observed, deleting session {session_id}",);
                    if queue_session_deletes {
                        state
                            .nats
                            .enqueue_session_delete(session_id.clone())
                        ...
                        info!(
                            "Mock mode: not queueing delete for disconnected session {session_id}"
                        );
                    }
                }
                if event.operation == Operation::Put {
                    info!("Active connection put observed for session {session_id}, deleting from unclaimed_sessions");
