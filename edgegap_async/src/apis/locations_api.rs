...
/// struct for typed errors of method [`location_beacon_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LocationBeaconListError {
...
pub async fn location_beacon_list(
    configuration: &configuration::Configuration,
) -> Result<models::LocationBeaconList, Error<LocationBeaconListError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/locations/beacons", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<LocationBeaconListError> =
...
pub async fn locations_get(
    ...
    r#type: Option<&str>,
    tags: Option<&str>,
) -> Result<models::Locations, Error<LocationsGetError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<LocationsGetError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
