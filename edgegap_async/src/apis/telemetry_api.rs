...
pub async fn active_deployment_telemetry_get(
    configuration: &configuration::Configuration,
    retrieval_key: &str,
) -> Result<models::ActiveDeploymentTelemetryGetResult, Error<ActiveDeploymentTelemetryGetError>> {
    ...
    let local_var_uri_str = format!(
        ...
        retrieval_key = crate::apis::urlencode(retrieval_key)
    );
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<ActiveDeploymentTelemetryGetError> =
...
pub async fn active_deployment_telemetry_post(
    configuration: &configuration::Configuration,
    payload: models::ActiveDeploymentTelemetryRequest,
) -> Result<models::ActiveDeploymentTelemetryResponse, Error<ActiveDeploymentTelemetryPostError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<ActiveDeploymentTelemetryPostError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
