...
/// struct for typed errors of method [`image_tag_delete`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ImageTagDeleteError {
    Status404(models::Error),
    Status401(models::Error),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`registry_image_tag_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RegistryImageTagListError {
    Status401(models::Error),
    Status404(models::Error),
...
pub async fn image_tag_delete(
    ...
    image_name: &str,
    tag_name: &str,
) -> Result<models::ApiModelRegistryartifacttagdeleteresponse, Error<ImageTagDeleteError>> {
    ...
    let local_var_uri_str = format!(
        ...
        image_name = crate::apis::urlencode(image_name),
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<ImageTagDeleteError> =
...
pub async fn registry_image_tag_list(
    configuration: &configuration::Configuration,
    image_name: &str,
) -> Result<models::ImageTagList, Error<RegistryImageTagListError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<RegistryImageTagListError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
