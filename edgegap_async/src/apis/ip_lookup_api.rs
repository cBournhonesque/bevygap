...
) -> Result<models::IpAddressResponse, Error<IPError>> {
...
pub async fn i_p_lookup(
    configuration: &configuration::Configuration,
    ip: &str,
) -> Result<models::IpAddressLookupResponse, Error<IPLookupError>> {
    let local_var_configuration = configuration;
...
pub async fn i_ps_lookup(
    configuration: &configuration::Configuration,
    payload: models::IpAddressesLookupPayload,
) -> Result<models::IpAddressesLookupResponse, Error<IPsLookupError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<IPsLookupError> =
